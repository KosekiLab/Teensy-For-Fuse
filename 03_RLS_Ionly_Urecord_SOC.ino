...
const int TSpin0 =  38; // State signal pin to check the performance in oscilloscope. Voltage signal.
...
// Basic settings
const int highDuration = 3000;  // Duration of high state in microseconds => IGBT turned on (current flows) for this time. Generally 2~3ms. need to discuss with Saitama uni.
...
const int window_size_2 = 10; // adjustable value
const int window_size = 20; // adjustable value
const double threshold1 = 250; // fault current threshold. adjustable value
...
double lambda = 0.96; // Forgetting factor(0.95~1, if it is going to be closer 1, more accurate but delayed) => only adjustable value in RLS parameters
...
void processData();
uint32_t FreeMem(); // check the left usable memory
...
void processData() {
    ...
    if (state == 1) {
        ...
        if (std::accumulate(peak_window.begin(), peak_window.begin() + 3, 0.0f) >
            std::accumulate(peak_window.end() - 3, peak_window.end(), 0.0f)) {
    ...
    if (state == 2){
        ...
        if (H > 1e-3 && A_estimate_rls > 5 && (A_estimate_rls - prev_A) < 0) {
