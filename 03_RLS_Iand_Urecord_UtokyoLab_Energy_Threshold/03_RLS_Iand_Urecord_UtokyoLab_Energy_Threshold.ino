...
// Set the pin numbers
const int readPin0 = A11; // Current sensing pin
const int readPin1 = A14; // Voltage sensing pin
...
const int window_size_2 = 20; // adjustable value
const int window_size = 10; // adjustable value
const double threshold1 = 0.5; // fault current threshold. adjustable value
double delta_T = 2e-5; // Time interval in seconds // adjustable, 1/freq
double ms_T = 2e-2; // Time interval in milliseconds // to record in SD card => delta_T*1000 = 1/freq*1000, adjustable
...
void writeDataToSDCard2() {
  ...
  if (dataFile) {
    ...
    dataFile.println("Time Since Surge,Post Surge Data,Voltage Data,Accumulated Energy,A Estimates RLS");
    ...
    while (dataIndex < post_surge_data.size()) {
        for (size_t i = dataIndex; i < std::min(dataIndex + WRITE_BATCH_SIZE, post_surge_data.size()); ++i) {
            ...
            dataFile.print(energy_data[i]);dataFile.print(",");
...
void loop() {
  ...
  if (adc0_ready && adc1_ready) {
    ...
    E = current * voltage;
    cummulated_energy = cummulated_energy + E;
...
void adc0_isr() {
  ...
  current = (adc_val0 * (3.3 / adc->adc0->getMaxValue())) ; // 2000 is the value for Rogowski coil in current sensor
...
void adc1_isr() {
    ...
    voltage = (adc_val1 * (3.3 / adc->adc1->getMaxValue()));
...
void processData() {
    ...
    if (state == 0) {
        ...
        if (allGreaterOrEqual(window_2, threshold1)) {
            ...
            energy_data.resize(voltage_data.size(),0.000000000000000f);
    ...
    if (state == 1) {
        ...
        if ((std::accumulate(peak_window.begin(), peak_window.begin() + 5, 0.0f) >
            std::accumulate(peak_window.end() - 5, peak_window.end(), 0.0f)) & (U > 1))  {
            ...
            voltage_data.push_back(U);
            energy_data.push_back(cummulated_energy);
